...
function mergeData(filename, data) {
  ...
  _.each(data, function(d) {
    ...
    preMerge[name].ons = d.ons || preMerge[name].ons;
  ...
  if (mergedCount == 4) {
    ...
    writeData(mergedData, finalOutputFile, ['name', 'ons', '06_infant_mortality','07_infant_mortality','08_infant_mortality','09_infant_mortality']);
  }
}
...
function parseWeirdCsv(inputFile, outputFile, startRow, diffToData, entityRowDiff, incOneList, done) {
  ...
  parse(rawText, function(err, rawData) {
    ...
    for (var i = startRow; i < rawData.length; i += entityRowDiff) {
      ...
      var ons = trim(rawData[i+diffToData][0]);
      var im = trim(rawData[i+diffToData][3]);
